...
- [~] After porting, remove the `[[redirects]]` blocks from `netlify.toml` to avoid drift between two sources of truth — Next.js redirects are the canonical source going forward _(done as part of the §1.11 netlify.toml rewrite)_
- [x] Port sitemap: replace `gatsby-plugin-sitemap` with `next-sitemap` configured to read from Sanity _(used native `app/sitemap.ts` (Sanity-driven) instead of next-sitemap — mirrors the Sanity reference template; satisfies FR-009 with no postbuild step)_
- [x] Port metadata: convert any `<Helmet>` / `<Head>` usage to App Router `metadata` exports / `generateMetadata` _(SEO.tsx OG/Twitter ported to layout defaults; per-route `generateMetadata` added with each page port — index/leadership/[slug] pending their ports)_
- [x] **Port GTM** (correction — analytics IS wired). _(next/script afterInteractive + noscript + GtmRouteTracker pushing 'gatsby-route-change'; GTM-debug verification is part of §1.12 on the branch deploy)_ `gatsby-config.ts:57` includes `gatsby-plugin-google-tagmanager` with container `GTM-5BVGJ4Q`. Replace with `next/script` in `app/layout.tsx` using `strategy="afterInteractive"`, plus the noscript iframe fallback in `<body>`. Verify pageview events fire on App Router route changes (Next does not emit them by default — wire up via `usePathname` + `useSearchParams` in a Client Component that calls `dataLayer.push`).
